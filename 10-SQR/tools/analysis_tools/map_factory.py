...
def main():
    ...
    outs5 = postproc_deform(pkl, 5, 1)
    outs4 = postproc_deform(pkl, 4, 1)
    outs3 = postproc_deform(pkl, 3, 1)
    outs2 = postproc_deform(pkl, 2, 1)
    outs1 = postproc_deform(pkl, 1, 1)
    outs0 = postproc_deform(pkl, 0, 1)

    # outs = organize(pkl)
    #outs5 = postproc(pkl, 5, 1)
    #outs4 = postproc(pkl, 4, 1)
    #outs3 = postproc(pkl, 3,1)
    #outs2 = postproc(pkl, 2,1)
    #outs1 = postproc(pkl, 1,1)
    #outs0 = postproc(pkl, 0,1)

    import numpy as np

    for iou_thrs in list(np.arange(0.5, 1, 0.05)):
        try:
            ...
            #cocoEval5.evaluate()
            #inter_r2 = cocoEval5.evalImgs
            #cocoEval5.accumulate()
            #cocoEval5.summarize()

            cocoEval5.evalImgs = fangyi(base=cocoEval5, supts=[cocoEval4,
                                                      cocoEval3,
                                                      cocoEval2,
                                                      cocoEval1,
                                                      cocoEval0,
            ...
            cocoEval5._paramsEval = copy.deepcopy(cocoEval5.params)
            ...
            if not len(ious) == 0:
                for tind, t in enumerate(p.iouThrs):
                    for dind, d in enumerate(dt):
                        ...
                        if m == -1:
                            FP2 += 1
                            ckfp1, ckfp2 = check_supts_FP(imgId, catId, d, supts, t)
                            if ckfp1:
                                grief1 += 1
                                grief += 1
                                dt[dind]['score'] = ckfp2
                        ...
                        if gtIg[m] == 0 and not (d['area'] < aRng[0] or d['area'] > aRng[1]):
                            TP2 += 1
                            cktp1, cktp2, cktp3, cktp4 = check_supts_TP(imgId, catId, g_reserve, d, supts, t)
                            if cktp1:
                                sorrow += 1
                                dt[dind]['bbox'] = cktp4['bbox']
                                dt[dind]['area'] = cktp4['area']
                                dt[dind]['score'] = cktp4['score']
                                dt[dind]['segmentation'] = cktp4['segmentation']
...
def check_supts_TP(imgId, catId, g, d, supts, t):
    ...
    return success, t, base_score, reserve
...
def postproc(outs, stageid=5, mmdet_implementation=False):
    ...
    for img in outs:
        ...
        cls_score_per_img = torch.sigmoid(torch.Tensor(cls_score))
        queryid = torch.Tensor(list(range(len(cls_score_per_img)))).float()
        if mmdet_implementation:
            ...
            queryid = queryid[topk_indices // 80]
        else:
            scores_per_img, labels_per_img = torch.max(cls_score_per_img, dim=1)
            bbox_pred_per_img = torch.Tensor(bboxes_list)
...
def postproc_deform(outs, stageid=5, mmdet_implementation=False):
    ...
    for img in outs:
        ...
        scale_factor = img['scale_factor']
        queryid = torch.Tensor(list(range(len(cls_score)))).float()

        if mmdet_implementation:
            scores, topk_indices = cls_score.flatten(0, 1).topk(100, sorted=False)
            det_labels = topk_indices % 80
            bbox_pred = torch.Tensor(bbox_pred)[topk_indices // 80]
            queryid = queryid[topk_indices // 80]
        else:
